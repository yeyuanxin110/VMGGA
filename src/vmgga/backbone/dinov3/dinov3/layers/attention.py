...
class SelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, q, k, v, attn_mask=None):
        """
        手动实现缩放点积注意力，等价于PyTorch 2.0+的同名函数
        Args:
            q: 查询张量，形状 [batch_size, num_heads, seq_len_q, d_k]
            k: 键张量，形状 [batch_size, num_heads, seq_len_k, d_k]
            v: 值张量，形状 [batch_size, num_heads, seq_len_v, d_v]（seq_len_k通常等于seq_len_v）
            attn_mask: 注意力掩码（可选），形状 [batch_size, num_heads, seq_len_q, seq_len_k]
        ...
        if attn_mask is not None:
            attn_scores = attn_scores.masked_fill(attn_mask == 0, -1e9)  # 掩码位置设为极小值
