...
class SelfAttentionBlock(nn.Module):
    ...
    @staticmethod
    def _maybe_index_rope(rope: Union[Tuple[Tensor, Tensor], None], indices: Tensor) -> Union[Tuple[Tensor, Tensor], None]:
        if rope is None:
            return None
...
class CausalSelfAttentionBlock(nn.Module):
    ...
    def init_weights(
        self,
        init_attn_std: Union[float, None] = None,
        init_proj_std: Union[float, None] = None,
        init_fc_std: Union[float, None] = None,
        factor: float = 1.0,
    ) -> None:
