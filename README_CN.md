...
论文中使用的自制 Optical-Map 数据集将通过外部网盘链接提供。

| 数据集 | 说明             | 谷歌云盘 | 百度网盘 |
|---|----------------|---|---|
| Optical-Map | 光学卫星影像与栅格地图图像对 | 待更新 | 待更新 |
...
在 `demo_vmgga.py` 中选择模态，放入对应的模型权重和示例图像，
然后直接运行 demo。脚本会自动使用该模态预设好的匹配设置。
...
以 optical-SAR 为例，将示例图像放入对应模态目录：
